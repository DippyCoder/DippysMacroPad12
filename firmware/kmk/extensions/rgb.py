...
def rgb_to_hsv(rgb):
    ...
    if delta == 0:
        ...
        h = (60 * ((b_f - r_f) / delta) + 120) % 360
    else:
        h = (60 * ((r_f - g_f) / delta) + 240) % 360
...
class RGB(Extension):
    ...
    def __init__(
        ...
        if self.default_color is not None:
            self.hue, self.sat, self.val = rgb_to_hsv(self.default_color)
            self.hue_default = self.hue
            self.sat_default = self.sat
            self.val_default = self.val
    ...
    def during_bootup(self, sandbox):
        ...
        if self.default_color is not None:
            self.set_rgb_fill(self.default_color)
            self.show()
